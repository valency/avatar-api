...
def find_candidates_from_road(road_network, point):
    candidates = set()
    unit_lat = (road_network.pmax.lat - road_network.pmin.lat) / road_network.grid_count
    unit_lng = (road_network.pmax.lng - road_network.pmin.lng) / road_network.grid_count
    if point.lat < road_network.pmin.lat or point.lng < road_network.pmin.lng or point.lat > road_network.pmax.lat or point.lng > road_network.pmax.lng:
        ...
        for i in range(lat_index - 1 if lat_index > 0 else 0, lat_index + 2 if lat_index + 1 < road_network.grid_count else road_network.grid_count):
            for j in range(lng_index - 1 if lng_index > 0 else 0, lng_index + 2 if lng_index + 1 < road_network.grid_count else road_network.grid_count):
